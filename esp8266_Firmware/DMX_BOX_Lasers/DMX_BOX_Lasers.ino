...
const int numDMXChannels=12;
const char *ssid = "labnetnode_mcu"; //HotSpot Password, use web UI to setup local wifi
const char *password = "mshort123";
bool startupMode=true;
bool wifiConnected=false;
ESP8266WebServer server(80);
struct {
  char ssid[256] = "pi4b-Wifi";
  char password[256] = "mshort123";
  char universe[32] = "1";
  char startChan[32] = "133";
}epdata;
...
char* getResponsePayoad(void){
  ...
  char responseString5[]="'; function submitForm(){payload='http://'+location.host + '/data/?ssid=' + document.getElementById('SSID').value + '&password=' + document.getElementById('password').value + '&universe=' + document.getElementById('universe').value + '&startChan=' + document.getElementById('startChannel').value;var xhttp = new XMLHttpRequest(); xhttp.onreadystatechange = function() { if(this.readyState == 4 && this.status == 200){console.log(this.responseText);}};xhttp.open('GET', payload, true);xhttp.send();}</script></body></html>";
